...
[~,wn4]=ellipord(2*ftr(1)/fs,2*ftr(2)/fs,rp,rs); % determine f0
n4 = 14; % set order to 14
[b4,a4,k]=ellip(n4,rp,rs,wn4); % design an elliptic filter
sos = zp2sos(b4,a4,k); % convert zeros and ports to sos
z4 = sosfilt(sos,y); % use sos to improve performance
[b3,a3]=ellip(n4,rp,rs,wn4); % keep the original filter without implementation
[snr4,ax4,e4,v4]=mb1_snrtone(z4,xfa,fs);  % find the filtered SNR, gain errors and residual noise
%
% phase response
%
[h4,w4] = freqz(b3, a3, fax*2*pi/fs); % deploy filter
angle4 = unwrap(angle(h4)); % obtain and unwrap angle
figure(2);
subplot(2,1,1);
plot(fax, angle4); % plot the phase response
...
%
figure(3);
zplane(b4,a4); % plot zeros and poles
