...
xlabel('Time (s)');
% 
% now design a Parks-McClellan optimal FIR filter with polyphase implementation
%
...
k=round(fs/2/mean(ftr)); % scaling factor k
z6(mod(0:nt-1,k)>0)=0;  % set k-1 out of every k samples to zero
z6=filter(h6,1,z6) * k; % filter again and multiply by k
...
xlabel('Frequency (Hz)');
ylabel('Gain (dB)');
title(sprintf('Optimal FIR Filter Order %d with polyphase implementation, SNR = %.1f dB',n6,snr6));
%
