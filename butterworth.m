...
% phase response
%
[h1,w1] = freqz(b1,a1,fax*2*pi/fs); % deploy filter
angle1 = unwrap(angle(h1)); % obtain and unwrap angle
figure(2);
subplot(2,1,1);
plot(fax, angle1); % plot the phase response
...
%
subplot(2,1,2);
grpdelay(b1, a1, fax*2*pi/fs); % plot the group delay
%
